...
function check_cpu_ivybridge() {
  local vmlinux="$1"
  local warn=0

  echo "[CPU CHECK] Scanning vmlinux for Haswell-only instructions..."

  AVX2=$(objdump -d "${vmlinux}" 2>/dev/null | grep -cE \
    "ymm[0-9]|vpbroadcast|vperm2i|vinserti|vpcmpeq|vpand|vpor|vpxor|vpsll|vpsrl" || true)

  BMI2=$(objdump -d "${vmlinux}" 2>/dev/null | grep -cE \
    "\brorx\b|\bshrx\b|\bshlx\b|\bsarx\b|\bpdep\b|\bpext\b|\bbzhi\b|\bmulx\b" || true)

  INVPCID=$(objdump -d "${vmlinux}" 2>/dev/null | grep -c "invpcid" || true)

  FMA=$(objdump -d "${vmlinux}" 2>/dev/null | grep -cE \
    "vfmadd|vfmsub|vfnmadd|vfnmsub" || true)

  printf "  AVX2    : %d hits\n" "${AVX2}"
  printf "  BMI2    : %d hits\n" "${BMI2}"
  printf "  INVPCID : %d hits\n" "${INVPCID}"
  printf "  FMA3    : %d hits\n" "${FMA}"
  ...
  if [ "${warn}" -eq 1 ]; then
    echo "[CPU CHECK] WARNING: Haswell-only instructions detected!"
    echo "[CPU CHECK]          This kernel will FREEZE on Ivy Bridge (3rd gen)."
    # 강제 중단하려면 아래 주석 해제
    # exit 1
  else
    echo "[CPU CHECK] OK: No Haswell-only instructions found (Ivy Bridge safe)"
  fi
}
...
check_cpu_ivybridge "${VMLINUX_MOD}"
