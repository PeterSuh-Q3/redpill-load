...
if [[ "${BRP_DEV_DISABLE_EXTS}" -ne 1 ]]; then
  ...
  # user_config.json 의 general.modulename 이 다른 모듈팩(amd-/custom-/anodrm-modules)을
  ...
  case "${BRP_UCF_MDLNAME}" in
    all-modules|custom-modules|anodrm-modules)
      ...
      # 혹시 IDS 에 없던 모듈팩 키가 연관배열에만 남아있을 경우까지 방어적으로 제거
      # 아울러 이전 빌드에서 디스크(custom/extensions/<id>)에 설치된 다른 모듈팩을
      # 제거한다. RPT_BUILD_EXTS 가 비면(USER_EXTS 미지정) _update_platform_exts 가
      ...
      for _mk in all-modules custom-modules anodrm-modules; do
        ...
        if [ -d "${_RPT_EXTS_DIR}/${_mk}" ]; then
          rm -rf "${_RPT_EXTS_DIR}/${_mk}"
          pr_process "[modules] removed stale installed module pack '%s'" "${_mk}"
          pr_empty_nl
        fi
      done
...
if [ ! -d "${BRP_UPAT_DIR}" ]; then
  ...
  for _brp_pat_f in "${BRP_PAT_SELECTIVE_FILES[@]}"; do
    if ! echo "${BRP_PAT_LISTING}" | grep -qE "(^|/)${_brp_pat_f}\$"; then
  ...
  if [[ "${BRP_PAT_SELECTIVE_OK}" -eq 1 ]]; then
    pr_process "[pat-selective] Unpacking only %s from %s to %s" "${BRP_PAT_SELECTIVE_FILES[*]}" "${BRP_PAT_FILE}" "${BRP_UPAT_DIR}"
    "${TAR_PATH}" -xf "${BRP_PAT_FILE}" -C "${BRP_UPAT_DIR}" "${BRP_PAT_SELECTIVE_FILES[@]}" \
