...
- **synoboot1/zImage** = 로더가 사용하는 커널. 기본은 Synology kpatched 커널이며,  
  `all-modules` / `amd-modules` 익스텐션이 활성화된 경우 `ext/official-zImage/` 의 커스텀 커널로 교체됨.
...
- **synoboot1/zImage** = Kernel used by the loader. Defaults to the Synology kpatched kernel.  
  When `all-modules` / `amd-modules` extensions are active, replaced with the custom kernel from `ext/official-zImage/`.
